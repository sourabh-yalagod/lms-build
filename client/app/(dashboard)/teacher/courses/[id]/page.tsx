...
const page = () => {
  ...
  const { data: course, isLoading, refetch } = useGetCourseByIdQuery({ id });
  ...
  const onSubmit = async (data: CourseFormData) => {
    if (!data) {
      toast.error(`Please consider all the Form fields....!`);
      return;
    }
    try {
      ...
      toast.error('Course Upadation failed.');
    }
  };
      ...
      <Form {...methods}>
        <form onSubmit={methods.handleSubmit(onSubmit)}>
          <Header
            ...
            rightElement={
              <div className="flex items-center transition-all space-x-4">
                <CustomFormField
                  name="courseStatus"
                  label={methods.watch('courseStatus') ? 'Published' : 'Draft'}
                  type="switch"
                  className="flex items-center duration-700 space-x-2"
                ...
                <Button
                  type="submit"
                  className="bg-primary-700 transition-all duration-700 hover:bg-primary-600"
                >
                  {methods.watch('courseStatus')
